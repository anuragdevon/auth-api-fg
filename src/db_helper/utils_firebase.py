...
def FirebaseInit():
    ...
    CRED = firebase_admin.credentials.Certificate(
        os.environ["GOOGLE_APPLICATION_CREDENTIALS_AUTH"]
    )
...
def UserDB_Update(uid, user_data):
    global FIREBASE_DB
    return (
        FIREBASE_DB.collection(CollectionName_Users)
        .document(uid)
...
def UserDB_UpdateArray(uid, field, element, add=True):
    ...
    return (
        FIREBASE_DB.collection(CollectionName_Users)
        .document(uid)
        .update({field: ArrayOperation([element])})
    )
...
def UserDB_AddSubcollection(uid, subcollection, docs={}):
    global FIREBASE_DB
    for k in docs.keys():
        FIREBASE_DB.collection(CollectionName_Users).document(uid).collection(
            subcollection
        ).document(k).create(docs[k])
    return None
...
def User_RegenerateIDToken(refresh_token):
    ...
    r = requests.post(REGEN_IDTOKEN_API_URL, data=payload)
...
def User_ResetPassword(email):
    ...
    payload = json.dumps({"requestType": "PASSWORD_RESET", "email": str(email)})

    response = requests.post(PASSWORD_RESET_API_URL, data=payload).json()
